...
<div class="container-fluid my-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 text-primary">إضافة طلب صيانة جديد</h1>
            <p class="text-muted mb-0">أدخل بيانات طلب الصيانة</p>
        </div>
            ...
            <form action="{{ route('admin.maintenances.store') }}" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="device_type" id="device_type" class="form-control @error('device_type') is-invalid @enderror"
                               value="{{ old('device_type') }}" placeholder="مثال: هاتف محمول، كمبيوتر محمول" required>
                        @error('device_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="fault_type" id="fault_type" class="form-control @error('fault_type') is-invalid @enderror"
                               value="{{ old('fault_type') }}" placeholder="وصف العطل" required>
                        @error('fault_type')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="notes" id="notes" class="form-control @error('notes') is-invalid @enderror"
                              rows="4" placeholder="أي ملاحظات إضافية">{{ old('notes') }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
